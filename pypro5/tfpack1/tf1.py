...
# 테너플로 자체는 기본적으로 C++로 구현이 되나 파이썬, 자바, 고(go) 등 다양한 언어를 지원한다.
# 하지만 파이썬을 최우선으로 지원하며 대부분의 편한기능들을 파이썬 라이브러리로만 구현되어 이써어서 python으로 개발하는 것을 추천~
...
print('GPU사용 가능' if tf.test.is_gpu_available() else 'GPU 사용 불가능 ㅠㅠ')
print(tf.config.list_physical_devices('GPU'))

print('tensor : 수치용 컨테이너다. 임의의 차원을 가지는 항렬의 일반화된 모습이다. 계산 그래프 구조를 갖는다. 병렬연산이 기본')
print(1, type(1))   # python
...
print(tf.constant([[1]]))  # 2-d tensor : matrix   -> 상수형 tensor
print()
a = tf.constant([1,2])
b = tf.constant([3,4])
c = a + b
